socket = io()

const listProducts = document.getElementById('products')

function appendProduct(product) {
    const newProduct = document.createElement('li')

    newProduct.innerHTML = `<strong>Title: </strong>${product.title}, <strong>Description: </strong>${product.description},
    <strong>Price: </strong>${product.price}, <strong>Thumbnail: </strong>${product.thumbnail}, <strong>Code: </strong>${product.code}, <strong>Status: </strong>${product.status}, <strong>Stock: </strong>${product.stock}`;
    listProducts.append(newProduct)
}


socket.on('products', products => {

    listProducts.innerHTML = ""
    products.map((product) => {
        appendProduct(product)
    })
})

